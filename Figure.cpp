...
Figure::Figure(Vertices &vertices, Vertices &texVertices)
{
	glGenVertexArrays(1, &vao);
	glBindVertexArray(vao);
	glBindBuffer(GL_ARRAY_BUFFER, vertices.vbo);
	...
	glBindBuffer(GL_ARRAY_BUFFER, texVertices.vbo);
...
void Figure::draw(Shaders &shaders, unsigned int texValue)
{
	...
	glBindVertexArray(vao);
	glDrawArrays(GL_TRIANGLES, 0, howManyTriangles);
	glUniform4f(Slots::colour, 1.0f, 1.0f, 1.0f, 1.0f);
}
...
void Figure::setColour(glm::vec4 &colour)
{
	glUniform4f(Slots::colour, colour.x, colour.y, colour.z, colour.a);
}